...
# generate dataset for sequential boolean vector encoder decoder
...
class Encoder(nn.Module):
    def __init__(self, input_dim, hid_dim, n_layers, dropout, retrain=False):
...
class Decoder(nn.Module):
    def __init__(self, output_dim, hid_dim, n_layers, dropout, retrain=False):
    ...
    def forward(self, trg, hidden, start=False):
        ...
        if self.training:
            ...
            start_token = torch.zeros(batch_size, 1, self.rnn_hid_dim)   # 创建一个全为0的向量，形状为 [batch_size, 1, hidden_size], 作为开始向量
            rnn_input = torch.cat((start_token, mlp_output[:, :-1]), dim=1) # 在第2维度（axis=1）的开始处concat全为0的向量
        else:
            if start:
                start_token = torch.zeros(batch_size, 1, self.rnn_hid_dim)
        ...
        outputs = outputs.view(outputs.size(0), outputs.size(1), self.output_dim, 2)
        predictions = self.softmax(outputs)

        return predictions, hidden
    
class Seq2Seq(nn.Module):
    def __init__(self, input_dim, output_dim, hid_dim, n_layers, dropout, retrain=False):
        super(Seq2Seq, self).__init__()
        self.encoder = Encoder(input_dim, hid_dim, n_layers, dropout, retrain)
        self.decoder = Decoder(output_dim, hid_dim, n_layers, dropout, retrain)
        
        if not retrain:
            model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'seq2seq_model_32bits.pth')
    ...
    def generate(self, src, max_len):
        ...
        trg = torch.zeros((src.size(0), 1, src.size(2)))
        decoder_outputs = torch.zeros(src.size(0), max_len, src.size(2))

        for i in range(0, max_len):
            output, hidden = self.decoder(trg, hidden, i==0)
            # output_binary = torch.argmax(output, dim=3).float()   # Error, cannot use argmax, or lose grad
            # output_binary = (output[:,:,:,1] > output[:,:,:,0]).float()
    ...
    def decode(self, hidden, src_shape):
        trg = torch.zeros((src_shape[0], 1, src_shape[2]))
        decoder_outputs = torch.zeros(src_shape[0], src_shape[1], src_shape[2])

        # src_shape is the length of the input sequence
        for i in range(0, src_shape[1]):
            output, hidden = self.decoder(trg, hidden, i==0)
            # output_binary = torch.argmax(output, dim=3).float()   # Error, cannot use argmax, or lose grad
            # output_binary = (output[:,:,:,1] > output[:,:,:,0]).float()   # Error, lose grad
            output_binary = output[:,:,:,1]
    ...
    def decode_test(self, hidden, src_shape, trg):
        decoder_outputs, _ = self.decoder(trg.float(), hidden)
        output_binary = decoder_outputs[:,:,:,1]
...
class trainer():
    ...
    def __init__(self, load_model=False):
        ...
        # 初始化模型、优化器和损失函数
        self.model = Seq2Seq(INPUT_DIM, OUTPUT_DIM, HID_DIM, N_LAYERS, DROPOUT, self.device, retrain=True).to(self.device)
...
def use_en_decoder():
    seq2seq_model = Seq2Seq(INPUT_DIM, OUTPUT_DIM, HID_DIM, N_LAYERS, DROPOUT)
    ...
    seq_len=10
    vec_len=32
    src = torch.randint(0, 2, (batch_size, seq_len, vec_len), dtype=torch.float32)
    
    encoded_hidden = seq2seq_model.encode(src)
    
    generated_output = seq2seq_model.decode(encoded_hidden, src.shape)
...
#     my_trainer = trainer()
#     my_trainer.train()
#     my_trainer.test(test_exist_model=True)
