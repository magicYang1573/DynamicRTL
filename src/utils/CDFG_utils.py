...
def get_node_type(name, op_to_index):
    ...
    if op_index == -1:
        error_message = f'[error] unknown node type: {type_of_name}'
...
def get_fanin_fanout(x_data, edge_index, edge_type):
    ...
    """
    def sort_fanin_list(fanin_list, edge_index, edge_type):
        edge_with_type = []
        for i in range(len(edge_index)):
            edge_with_type.append([edge_index[i], edge_type[i]])
        
        for i in range(len(fanin_list)):
            fanin_list_with_type = [[x[0][0], x[1]] for x in edge_with_type if x[0][1] == i]
            fanin_list_with_type = sorted(fanin_list_with_type, key=lambda x: x[1])
            fanin_list[i] = [x[0] for x in fanin_list_with_type]
    ...
    # zhou add 2024.09.14
    #############################
    sort_fanin_list(fanin_list, edge_index, edge_type)
    #############################
...
def check_node_name(names):
    def report_error(name):
        error_message = f'[error] invalid node name: {name}'
        raise ValueError(error_message)
...
def conststr2hexstr(conststr):
    if not "\'" in conststr:
        ...
        if system == 'h':
            ...
            error_message = f'[error] unknown system: {system}, value: {conststr}'
            raise ValueError(error_message)
        ...
        for name in name2idx.keys():
            if 'Constant' in name:
                if not check_conststr(name.split('_')[-1].lower()):
                    # print(f"[WARNING] Invalid char exists in simulation result, The string is: {name.split('_')[-1]}")
                    # return None
                    error_message = f'[error] Invalid char exists in simulation result, The string is: {name.split("_")[-1]}'
                    raise ValueError(error_message)
...
def remove_redundant_nodes_no_sim_res(x_data, edge_index, edge_type, name2idx, op_to_index):
...
def check_all_data(x_data, edge_index, edge_type, sim_res, name2idx, args):
    ...
    assert(max(edge_type) <=  args.max_edge_type)
...
def parse_design(CDFG_path, sim_path, op_to_index, args):
    ...
    if sim_res == None or len(sim_res) == 0:
    ...
    check_all_data(x_data, edge_index, edge_type, sim_res, name2idx, args)
